...
def get_safe_wandb_artifact_name(name: str):
    """Return a deterministic artifact name accepted by WandB.

    Artifact names are limited to 128 alphanumeric, dash, underscore, and dot
    characters. Dataset identifiers may be lists for multi-dataset training, so
    sanitize the complete name instead of handling only ``:`` and ``/``.
    """
    ...
    return f"{safe_name[:57]}__{digest}__{safe_name[-55:]}"
