...
class DatasetConfig:
    ...
    # Keep list first: draccus tries union decoders in annotation order, so JSON CLI lists
    # must be attempted before the permissive string decoder.
    ...
    root: str | None = None
    # A list selects episodes for one dataset. Multi-dataset training uses a mapping from repo id
    # to episode indices. Selection is read-only and does not rewrite the source datasets.
    ...
    exclude_episodes: list[int] | dict[str, list[int]] | None = None
    # Deterministically retain at most this many episodes from every dataset, after applying the
    # explicit include/exclude lists above.
    max_episodes_per_dataset: int | None = None
    episode_sampling_seed: int = 0
    # In multi-dataset mode, project these vector features to the ordered intersection of their
    # named dimensions. This supports mixing joint-only and joint+EEF datasets without rewriting.
    common_feature_keys: list[str] = field(default_factory=lambda: ["observation.state", "action"])
    # Rename source feature keys before computing the multi-dataset intersection. This is useful
    # for schema aliases and typos and does not modify files on disk.
    ...
    def __post_init__(self) -> None:
        repo_ids = [self.repo_id] if isinstance(self.repo_id, str) else self.repo_id
        if not repo_ids:
            raise ValueError("repo_id must contain at least one dataset")
        if len(repo_ids) != len(set(repo_ids)):
            duplicates = sorted({repo_id for repo_id in repo_ids if repo_ids.count(repo_id) > 1})
            raise ValueError(f"Duplicate dataset repo ids: {duplicates}")
        if self.repo_type not in ("dataset", "bucket"):
            raise ValueError(f"repo_type must be 'dataset' or 'bucket', got {self.repo_type!r}")
        if len(repo_ids) > 1 and self.repo_type == "bucket":
            raise ValueError("Multi-dataset training does not support repo_type='bucket'.")
        if len(repo_ids) > 1 and self.streaming:
            raise ValueError("Multi-dataset training does not support streaming datasets.")
        ...
        if len(repo_ids) > 1 and self.eval_split != 0.0:
            raise ValueError("eval_split is not yet supported for multi-dataset training.")
        ...
        if len(repo_ids) == 1 and isinstance(self.episodes, dict):
            raise ValueError("A single dataset expects episodes as a list, not a mapping.")
        if len(repo_ids) > 1 and isinstance(self.episodes, list):
            raise ValueError("Multiple datasets expect episodes as a mapping from repo id to indices.")
        if len(repo_ids) == 1 and isinstance(self.exclude_episodes, dict):
            raise ValueError("A single dataset expects exclude_episodes as a list, not a mapping.")
        if len(repo_ids) > 1 and isinstance(self.exclude_episodes, list):
            raise ValueError(
                "Multiple datasets expect exclude_episodes as a mapping from repo id to indices."
            )

        def validate_episode_mapping(
            value: list[int] | dict[str, list[int]] | None, field_name: str, *, ignore_negative: bool
        ) -> list[int] | dict[str, list[int]] | None:
            if value is None:
                return None
            mapping = {repo_ids[0]: value} if isinstance(value, list) else value
            unknown = sorted(set(mapping).difference(repo_ids))
            if unknown:
                raise ValueError(f"Unknown dataset ids in {field_name}: {unknown}")
            cleaned: dict[str, list[int]] = {}
            for repo_id, indices in mapping.items():
                negative = [index for index in indices if index < 0]
                if negative and not ignore_negative:
                    raise ValueError(f"Episode indices must be non-negative, got: {negative}")
                if negative:
                    if len(repo_ids) == 1:
                        logger.warning("Ignoring negative %s entries: %s", field_name, negative)
                    else:
                        logger.warning(
                            "Ignoring negative %s entries for %s: %s", field_name, repo_id, negative
                        )
                valid = [index for index in indices if index >= 0]
                if len(valid) != len(set(valid)):
                    duplicates = sorted({index for index in valid if valid.count(index) > 1})
                    raise ValueError(f"Episode indices contain duplicates: {duplicates}")
                cleaned[repo_id] = valid
            return cleaned[repo_ids[0]] if isinstance(value, list) else cleaned

        self.episodes = validate_episode_mapping(self.episodes, "episodes", ignore_negative=False)
        self.exclude_episodes = validate_episode_mapping(
            self.exclude_episodes, "exclude_episodes", ignore_negative=True
        )
        if self.max_episodes_per_dataset is not None and self.max_episodes_per_dataset < 1:
            raise ValueError("max_episodes_per_dataset must be at least 1")
        if len(self.common_feature_keys) != len(set(self.common_feature_keys)):
            raise ValueError("common_feature_keys must not contain duplicates")
