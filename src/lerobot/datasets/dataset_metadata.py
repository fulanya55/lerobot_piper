...
from .legacy_v21 import V21, load_legacy_episodes, load_legacy_stats, load_legacy_tasks
from .utils import (
...
class LeRobotDatasetMetadata:
    ...
    def _load_metadata(self):
        ...
        if self.is_legacy_v21:
            logging.warning(
                "Loading LeRobot dataset %s in read-only v2.1 compatibility mode. "
                "Recording, resuming, and pushing this dataset still require v3.0.",
                self.repo_id,
            )
            self.tasks = load_legacy_tasks(self.root) if self.total_tasks > 0 else None
            self.episodes = (
                load_legacy_episodes(self.root, self.fps, self.video_keys)
                if self.total_episodes > 0
                else None
            )
            self.stats = load_legacy_stats(self.root)
            return
    ...
    @property
    def is_legacy_v21(self) -> bool:
        """Whether this metadata uses the read-only LeRobot v2.1 layout."""
        return self.info.codebase_version == V21
    ...
    def get_data_file_path(self, ep_index: int) -> Path:
        ...
        """
        if self.is_legacy_v21:
            episode_chunk = ep_index // self.chunks_size
            return Path(self.data_path.format(episode_chunk=episode_chunk, episode_index=ep_index))
        if self.episodes is None:
