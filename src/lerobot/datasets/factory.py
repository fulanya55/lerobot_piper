...
def _root_for_repo(root: str | None, repo_id: str, *, multi: bool) -> str | None:
    if root is None:
        return None
    return str(Path(root) / repo_id) if multi else root


def _resolve_and_sample_episodes(
    *,
    ...
    episodes = resolve_episode_indices(requested, total_episodes, excluded)
    if episodes is None:
        if max_episodes is None or total_episodes <= max_episodes:
            return None
        episodes = list(range(total_episodes))
    if max_episodes is None or len(episodes) <= max_episodes:
        return episodes
    rng = np.random.default_rng(np.random.SeedSequence([seed, dataset_index]))
    selected = rng.choice(episodes, size=max_episodes, replace=False)
...
def make_dataset(cfg: TrainPipelineConfig) -> LeRobotDataset | MultiLeRobotDataset:
    ...
    if isinstance(cfg.dataset.repo_id, str):
        ...
        episodes = _resolve_and_sample_episodes(
            requested=cfg.dataset.episodes,
            excluded=cfg.dataset.exclude_episodes,
            total_episodes=ds_meta.total_episodes,
            max_episodes=cfg.dataset.max_episodes_per_dataset,
            seed=cfg.dataset.episode_sampling_seed,
            dataset_index=0,
        )
        ...
        requested_by_repo = cfg.dataset.episodes or {}
        excluded_by_repo = cfg.dataset.exclude_episodes or {}
        metadata = {
            repo_id: LeRobotDatasetMetadata(
                repo_id,
                root=_root_for_repo(cfg.dataset.root, repo_id, multi=True),
                revision=cfg.dataset.revision,
                repo_type=cfg.dataset.repo_type,
            )
        ...
        episodes = {
            ...
            if (
                selected := _resolve_and_sample_episodes(
                    requested=requested_by_repo.get(repo_id),
                    excluded=excluded_by_repo.get(repo_id),
                    total_episodes=metadata[repo_id].total_episodes,
                    max_episodes=cfg.dataset.max_episodes_per_dataset,
                    seed=cfg.dataset.episode_sampling_seed,
        ...
        delta_timestamps_by_repo = {
            repo_id: resolve_delta_timestamps(cfg.trainable_config, metadata[repo_id]) for repo_id in repo_ids
        }
        image_resolution = getattr(cfg.trainable_config, "image_resolution", None)
        dataset = MultiLeRobotDataset(
            repo_ids,
            root=cfg.dataset.root,
            episodes=episodes or None,
            delta_timestamps_by_repo=delta_timestamps_by_repo,
            image_transforms=image_transforms,
            video_backend=cfg.dataset.video_backend,
            common_feature_keys=cfg.dataset.common_feature_keys,
            feature_rename_map=cfg.dataset.feature_rename_map,
            image_size=tuple(image_resolution) if image_resolution is not None else None,
            revision=cfg.dataset.revision,
            return_uint8=True,
            depth_output_unit=cfg.dataset.depth_output_unit,
