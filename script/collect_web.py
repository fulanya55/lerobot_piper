...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path=='/api/status':
            ...
            message = '采集中…' if capture_status == 'running' else ('等待下一条 episode（按空格开始）' if capture_status == 'waiting' else ('正在初始化同步采集…' if capture_status == 'starting' else '服务就绪'))
            self._json({'running':capture_status == 'running','services':'up' if services_proc is not None and services_proc.poll() is None else 'down','preview':'up' if preview_up else 'down','capture':capture_status,'message':message,'log':tail_logs()}); return
