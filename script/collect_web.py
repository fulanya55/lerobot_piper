...
def start_services():
    global services_proc, preview_proc
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path=='/api/status':
            self._json({'running':capture_proc is not None and capture_proc.poll() is None,'services':'up' if services_proc is not None and services_proc.poll() is None else 'down','preview':'up' if preview_proc is not None and preview_proc.poll() is None else 'down','capture':'running' if capture_proc is not None and capture_proc.poll() is None else 'idle','message':'采集中…' if capture_proc is not None and capture_proc.poll() is None else '服务就绪','log':tail_logs()}); return
    ...
    def do_POST(self):
        ...
        if self.path=='/api/start':
            ...
            b=json.loads(self.rfile.read(int(self.headers.get('Content-Length',0)))); out=capture_log.open('w'); args=[str(DIRECT),'--skip-can','--yes','--auto-start']
            for k in ('dataset_path','repo_id','task','episode_idx','timesteps','fps','camera_resolution'): args += ['--'+k.replace('_','-'),str(b[k])]
            capture_proc=subprocess.Popen(args,cwd=ROOT,start_new_session=True,stdout=out,stderr=subprocess.STDOUT); self._json({'message':'采集已启动，正在等待同步帧'}); return
