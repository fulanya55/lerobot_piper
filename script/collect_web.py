...
HTML = HTML.replace("let running=false,logFollow=true;", "let running=false,logFollow=true,episodeRenderKey='';")
...
def list_datasets():
    """Return local LeRobot datasets that can be resumed by the UI."""
    result = []
    ...
    for info_path in sorted(DATASET_ROOT.rglob("meta/info.json")):
        try:
            ...
            if info.get("codebase_version") != "v2.1" or not info.get("data_path"):
                continue
            root = info_path.parent.parent.resolve()
            result.append({
                "name": str(root.relative_to(DATASET_ROOT.resolve())),
                "path": str(root),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path == '/api/episodes':
            ...
            center = int(q.get('center', [0])[0]); rows = []
            info_path = root / 'meta/info.json'
            try:
                info = json.loads(info_path.read_text(encoding='utf-8')); total = int(info.get('total_episodes', 0)); fps = float(info.get('fps', 30))
                if info.get('codebase_version') == 'v2.1':
                    records = [json.loads(line) for line in (root/'meta/episodes.jsonl').read_text(encoding='utf-8').splitlines() if line.strip()]
                    window_start = min(max(center - 1, 0), max(total - 3, 0))
                    for rec in records[window_start:window_start + min(3, total)]:
                        ep = int(rec['episode_index']); chunk = ep // int(info.get('chunks_size', 1000)); length = int(rec['length'])
                        videos = []
                        for key in ('cam_high','cam_left_wrist','cam_right_wrist'):
                            rel = Path('videos')/f'chunk-{chunk:03d}'/f'observation.images.{key}'/f'episode_{ep:06d}.mp4'
                            videos.append({'name': key, 'url': '/media/'+quote(str(rel))+'?dataset='+quote(str(root))})
                        rows.append({'episode': ep, 'length': length, 'duration': length/fps, 'task': (rec.get('tasks') or [''])[0], 'videos': videos})
                else:
