...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path=='/api/status':
            ...
            self._json({'running':active and state == 'recording','services':'up' if services_proc is not None and services_proc.poll() is None else 'down','preview':'up' if preview_up else 'down','capture':'running' if active and state == 'recording' else ('waiting' if active else 'idle'),'message':'采集中…' if active and state == 'recording' else ('等待下一条 episode（按空格开始）' if active else '服务就绪'),'log':tail_logs()}); return
