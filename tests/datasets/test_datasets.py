...
def test_multilerobot_dataset_resizes_rgb_with_centered_padding(
    tmp_path, lerobot_dataset_factory, info_factory
):
    root = tmp_path / "multi_padded_resize"
    repo_ids = ["first", "second"]
    camera_key = "observation.images.camera"
    camera_feature = {"shape": (32, 48, 3), "names": ["height", "width", "channels"]}
    for repo_id in repo_ids:
        info = info_factory(
            total_episodes=2,
            total_frames=12,
    ...
    source_height, source_width = dataset._datasets[0][0][camera_key].shape[-2:]
    ...
    image = dataset[0][camera_key]

    assert image.shape == (3, target_height, target_width)
    if pad_top:
        torch.testing.assert_close(image[:, :pad_top], torch.zeros_like(image[:, :pad_top]))
    if pad_bottom:
        torch.testing.assert_close(image[:, -pad_bottom:], torch.zeros_like(image[:, -pad_bottom:]))
    if pad_left:
        torch.testing.assert_close(image[:, :, :pad_left], torch.zeros_like(image[:, :, :pad_left]))
    if pad_right:
        torch.testing.assert_close(image[:, :, -pad_right:], torch.zeros_like(image[:, :, -pad_right:]))
    assert torch.count_nonzero(
        image[
            :,
            pad_top : target_height - pad_bottom,
            pad_left : target_width - pad_right,
        ]
    ) > 0


def test_multilerobot_dataset_projects_named_vector_features(tmp_path, lerobot_dataset_factory, info_factory):
    root = tmp_path / "multi_projected"
    repo_ids = ["first", "second"]
    common_names = ["left_joint", "right_joint"]
    motor_features = [
        {
            OBS_STATE: {"dtype": "float32", "shape": (2,), "names": common_names},
            ACTION: {"dtype": "float32", "shape": (2,), "names": common_names},
        },
        {
            OBS_STATE: {
                "dtype": "float32",
                "shape": (3,),
                "names": ["left_joint", "left_eef", "right_joint"],
            },
            ACTION: {
                "dtype": "float32",
                "shape": (3,),
    ...
    children = []
    for repo_id, features in zip(repo_ids, motor_features, strict=True):
        info = info_factory(
            total_episodes=2,
            total_frames=12,
            total_tasks=1,
            motor_features=features,
            use_videos=False,
        )
        children.append(
            lerobot_dataset_factory(
    ...
    dataset = MultiLeRobotDataset(
        ...
        root=root,
        download_videos=False,
        common_feature_keys=[OBS_STATE, ACTION],
    )

    assert dataset.meta.features[OBS_STATE]["names"] == common_names
    assert dataset.meta.features[ACTION]["names"] == common_names
    assert dataset.meta.features[OBS_STATE]["shape"] == (2,)
    assert dataset.meta.stats[OBS_STATE]["mean"].shape == (2,)
    assert dataset.meta.stats[ACTION]["std"].shape == (2,)
    assert dataset[0][OBS_STATE].shape == (2,)
    assert dataset[len(children[0])][ACTION].shape == (2,)
    assert dataset.num_episodes == sum(child.num_episodes for child in children)
    batch = next(iter(torch.utils.data.DataLoader(dataset, batch_size=len(children[0]) + 1)))
    assert batch[OBS_STATE].shape[-1] == 2
    assert batch[ACTION].shape[-1] == 2
    assert set(batch["dataset_index"].tolist()) == {0, 1}


def test_multilerobot_dataset_drops_undeclared_columns_before_collation(
    tmp_path, lerobot_dataset_factory
):
    root = tmp_path / "multi_extra_column"
    repo_ids = ["with_extra", "without_extra"]
    for repo_id in repo_ids:
        lerobot_dataset_factory(
            root=root / repo_id,
            repo_id=repo_id,
    ...
    dataset = MultiLeRobotDataset(repo_ids, root=root, download_videos=False)
    child_with_extra = dataset._datasets[0]
    reader = child_with_extra._ensure_reader()
    reader.load_and_activate()
    reader.hf_dataset = reader.hf_dataset.add_column(
        "subtask_indices", [[0]] * len(reader.hf_dataset)
    )

    assert "subtask_indices" not in dataset[0]
    batch = next(
        iter(torch.utils.data.DataLoader(dataset, batch_size=len(child_with_extra) + 1))
    )
    assert "subtask_indices" not in batch
    assert set(batch["dataset_index"].tolist()) == {0, 1}


def test_multilerobot_dataset_renames_camera_aliases_before_intersection(
    tmp_path, lerobot_dataset_factory, info_factory
):
    root = tmp_path / "multi_camera_aliases"
    repo_ids = ["high_camera", "front_camera"]
    source_camera_keys = [
        [
    ...
    camera_feature = {"shape": (32, 48, 3), "names": ["height", "width", "channels"]}
    for repo_id, camera_keys in zip(repo_ids, source_camera_keys, strict=True):
        info = info_factory(
            total_episodes=2,
            total_frames=12,
            total_tasks=1,
            camera_features=dict.fromkeys(camera_keys, camera_feature),
            use_videos=False,
        )
        lerobot_dataset_factory(root=root / repo_id, repo_id=repo_id, info=info, use_videos=False)

    rename_map = {
        "observation.images.cam_high_rgb": "observation.images.cam_front_rgb",
        "observation.images.cam_left_wrist_rgb_rgb": "observation.images.cam_left_wrist_rgb",
        "observation.images.cam_right_wrist_rgb_rgb": "observation.images.cam_right_wrist_rgb",
    }
    dataset = MultiLeRobotDataset(
        repo_ids,
        root=root,
    ...
    assert set(dataset.camera_keys) == expected
    for index in (0, len(dataset._datasets[0])):
        item = dataset[index]
        assert expected.issubset(item)
        assert not set(rename_map).intersection(item)


def test_multilerobot_dataset_uses_selected_episodes(tmp_path, lerobot_dataset_factory):
    root = tmp_path / "multi_episodes"
    repo_ids = ["first", "second"]
    for repo_id in repo_ids:
        lerobot_dataset_factory(
            root=root / repo_id,
    ...
    selected = {"first": [0, 2], "second": [1]}
    dataset = MultiLeRobotDataset(repo_ids, root=root, episodes=selected, download_videos=False)

    assert [child.episodes for child in dataset._datasets] == [[0, 2], [1]]
    assert dataset.num_episodes == 3
    assert dataset.meta.episodes["source_episode_index"] == [0, 2, 1]
    assert dataset.meta.episodes["dataset_from_index"][0] == 0
    assert dataset.meta.episodes["dataset_to_index"][-1] == len(dataset)


def test_factory_builds_multi_dataset_with_deterministic_episode_cap(tmp_path, lerobot_dataset_factory):
    root = tmp_path / "multi_factory"
    repo_ids = ["first", "second"]
    for repo_id in repo_ids:
        lerobot_dataset_factory(
            root=root / repo_id,
    ...
    dataset_config = DatasetConfig(
        ...
        episode_sampling_seed=123,
        common_feature_keys=[ACTION],
    )
    cfg = TrainPipelineConfig(dataset=dataset_config, policy=make_policy_config("act"))

    first = make_dataset(cfg)
    second = make_dataset(cfg)

    assert isinstance(first, MultiLeRobotDataset)
    assert first.num_episodes == 4
    assert all(len(child.episodes) == 2 for child in first._datasets)
    assert [child.episodes for child in first._datasets] == [child.episodes for child in second._datasets]
